...
from bot.database.db_config import get_safe_session, TripGroup
...
async def morning_briefing(context: ContextTypes.DEFAULT_TYPE):
    """Runs every day at 7:00 AM IST to send morning updates to all active trips."""
    ...
    try:
        async with get_safe_session() as session:
            ...
            for group in groups:
                try:
                    msg = (
                        f"🌅 <b>Good Morning, Expedition!</b>\n\n"
                        "It's 7:00 AM. Don't forget to check your <code>/weather</code> "
                        "and <code>/packing</code> list for today's adventures! 🎒🗺️"
                    )

...
from bot.database.db_config import get_safe_session, TripDocument, User, TripGroup
...
async def save_to_vault(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Detect media groups to prevent duplicate confirmation messages
    silent = False
    if update.message.media_group_id:
        group_id = update.message.media_group_id
        # ✅ FIX: Only delete media_group tracker keys, not all bot_data.
        # The old context.bot_data.clear() would wipe unrelated data stored
        # by other parts of the bot whenever someone uploaded 200+ photos.
        if len(context.bot_data) > 200:
            keys_to_delete = [k for k in context.bot_data.keys() if k.startswith("mg_")]
    ...
    try:
        async with get_safe_session() as session:
            docs = (await session.execute(
                select(TripDocument).where(TripDocument.chat_id == chat_id, TripDocument.file_type == "doc")
            )).scalars().all()
...
async def get_vault_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not doc or doc.chat_id != query.message.chat_id:
            return
